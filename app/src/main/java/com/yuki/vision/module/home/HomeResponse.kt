 ...
 */
data class HomeResponse(val issueList : ArrayList<Issue>, val nextPageUrl : String, val nextPublishTime : Long, val newestIssueType : String?, val dialog : Any) {
	...
	data class Issue(val releaseTime : Long, val type : String, val date : Long, val total : Int, val publishTime : Long, val itemList : ArrayList<Item>, var count : Int, val nextPageUrl : String?) {
		...
		data class Item(internal var itemType : Int = 0, val type : String, val data : Data?, val tag : String) : Serializable, MultiItemEntity {
			...
			data class Data(val dataType : String,
					...
					val playlists : Any,
					val header : Header,
					val itemList : ArrayList<HomeResponse.Issue.Item>
			) : Serializable {
