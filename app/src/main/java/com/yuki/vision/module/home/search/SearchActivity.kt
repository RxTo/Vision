...
import com.yuki.vision.module.home.search.di.DaggerSearchComponent
import com.yuki.vision.module.home.search.di.SearchModule
import com.yuki.vision.utils.StatusBarUtil
import com.yuki.vision.utils.ViewAnimUtils
...
class SearchActivity : XActivity<SearchPresenter>() {
	...
	@Inject
	lateinit var searchAdapter : CategoryDetailAdapter
	@Inject
	...
	private val mEmptyView : View by lazy { View.inflate(this, R.layout.view_empty, null) }
	private val mErrorView : View by lazy { View.inflate(this, R.layout.view_error, null) }
	private val mLoadView : View by lazy { View.inflate(this, R.layout.view_loading, null) }
	private val headerView by lazy { View.inflate(this, R.layout.item_search_header, null) }
	...
	override fun initView(savedInstanceState : Bundle?) {
		...
		searchAdapter.setLoadMoreView(loadMore)
	...
	override fun initEvent() {
		hotSearchAdapter.setOnItemClickListener { adapter, view, position ->
			keyWords = hotSearchAdapter.getItem(position).toString()
		...
		searchAdapter.setOnItemClickListener { adapter, view, position ->
			goToVideoPlayer(this,view, searchAdapter.getItem(position))
		}
		searchAdapter.setOnLoadMoreListener({
			mPresenter?.requestMoreIssue()
		}, mRecyclerView_result)
	...
	fun initHotWord(data : ArrayList<String>?) {
		...
		mRecyclerView_hot.adapter = hotSearchAdapter
	...
	fun initSearchResult(issue : HomeResponse.Issue) {
		...
		searchAdapter.setNewData(issue.itemList)
	}
	
	fun initMoreSearchResult(issue : Issue) {
		searchAdapter.loadMoreComplete()
		searchAdapter.addData(issue.itemList)
	}
